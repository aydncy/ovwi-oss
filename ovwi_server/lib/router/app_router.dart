...
Future<Router> buildRouter(Connection connection) async {
  ...
  Future<void> logUsage(String developerId, String endpoint, String method, int statusCode, int latency) async {
    try {
      ...
      print('✓ Usage: $developerId $method $endpoint ($statusCode) ${latency}ms');
    } catch (e) {
  ...
  router.post('/api/v1/developers', (Request request) async {
    try {
      ...
      final result = await connection.execute(Sql.named('INSERT INTO developers (id, name, email, company, plan, status, created_at, updated_at) VALUES (gen_random_uuid(), @name, @email, @company, @plan, @status, NOW(), NOW()) RETURNING id, name, email, company, plan, status, created_at, updated_at'), parameters: {'name': name, 'email': email, 'company': company, 'plan': 'free', 'status': 'active'});
  ...
  router.get('/api/v1/dashboard/keys', (Request request) async {
    ...
    try {
      ...
      if (result.isEmpty) {
        ...
        logUsage('unknown', '/api/v1/dashboard/keys', 'GET', 401, latency);
        return Response(401, headers: {'Content-Type': 'application/json'}, body: jsonEncode({'success': false, 'error': {'code': 'invalid_api_key', 'message': 'Invalid API key'}}));
      }
      ...
      if (storedPrefix != prefix) {
        final latency = DateTime.now().difference(startTime).inMilliseconds;
        logUsage(developerId, '/api/v1/dashboard/keys', 'GET', 401, latency);
        return Response(401, headers: {'Content-Type': 'application/json'}, body: jsonEncode({'success': false, 'error': {'code': 'invalid_api_key', 'message': 'Invalid API key'}}));
      }
      ...
      await connection.execute(Sql.named('UPDATE api_keys SET last_used_at = NOW() WHERE id = @id'), parameters: {'id': row[0]}).catchError((_) => null);

      return Response(200, headers: {'Content-Type': 'application/json'}, body: jsonEncode({'success': true, 'data': keysList}));
    } catch (e) {
      final latency = DateTime.now().difference(startTime).inMilliseconds;
  ...
  router.get('/api/v1/dashboard/usage', (Request request) async {
    ...
    try {
      ...
      if (result.isEmpty) {
        final latency = DateTime.now().difference(startTime).inMilliseconds;
        logUsage('unknown', '/api/v1/dashboard/usage', 'GET', 401, latency);
        return Response(401, headers: {'Content-Type': 'application/json'}, body: jsonEncode({'success': false, 'error': {'code': 'invalid_api_key', 'message': 'Invalid API key'}}));
      }
      ...
      if (storedPrefix != prefix) {
        final latency = DateTime.now().difference(startTime).inMilliseconds;
        logUsage(developerId, '/api/v1/dashboard/usage', 'GET', 401, latency);
        return Response(401, headers: {'Content-Type': 'application/json'}, body: jsonEncode({'success': false, 'error': {'code': 'invalid_api_key', 'message': 'Invalid API key'}}));
      }

      final usage = await connection.execute(
        Sql.named('SELECT COUNT(*) as total, SUM(CASE WHEN status_code < 400 THEN 1 ELSE 0 END) as success, SUM(CASE WHEN status_code >= 400 THEN 1 ELSE 0 END) as errors, AVG(latency_ms) as avg_latency FROM api_usage WHERE developer_id = @developer_id'),
        parameters: {'developer_id': developerId}
      );
      ...
      return Response(200, headers: {'Content-Type': 'application/json'}, body: jsonEncode({'success': true, 'data': {'total_requests': total, 'success_requests': success, 'error_requests': errors, 'avg_latency_ms': avgLatency.toStringAsFixed(2)}}));
    } catch (e) {
      final latency = DateTime.now().difference(startTime).inMilliseconds;
