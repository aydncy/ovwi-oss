...
class AnchorChainManager {
  ...
  Future<AnchorBlock> addEvent(String eventId, Map<String, dynamic> eventData, String signature) async {
    ...
    print('[ANCHOR] Event ' + eventId + ' anchored at sequence ' + block.sequence.toString());
    return block;
  }
...
void main() async {
  ...
  Middleware logRequests() => (Handler innerHandler) => (Request request) async {
    ...
    print('[REQUEST] ' + request.method + ' ' + request.url.toString());
  ...
  router.get('/health', (Request req) => Response.ok(jsonEncode({'status': 'healthy', 'version': '3.0.0', 'timestamp': DateTime.now().toUtc().toIso8601String(), 'chain_height': chainManager.getChain().length, 'architecture': 'event-driven'}), headers: {'Content-Type': 'application/json'}));

  router.get('/metrics', (Request req) {
    final apiKey = req.headers['x-api-key'] ?? 'unknown';
    return Response.ok(jsonEncode({'api_key_metrics': apiKeyManager.getKeyMetrics(apiKey), 'chain_stats': {'height': chainManager.getChain().length, 'integrity_valid': chainManager.verifyChainIntegrity(), 'last_block': chainManager.getChain().isNotEmpty ? chainManager.getChain().last.toJson() : null}}), headers: {'Content-Type': 'application/json'});
  });
  ...
  print('======================================');
  print('OVWI Server v3.0 (Event-Driven)');
  print('http://localhost:' + port.toString());
  print('Architecture: Anchor Chain + Event Bus');
